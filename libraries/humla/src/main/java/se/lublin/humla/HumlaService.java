...
/*perbaikan2*/
    ...
    public void onConnectionSynchronized() {
        ...
        if (mModelHandler == null) {
            Log.e(TAG, "onConnectionSynchronized: model null");
            return;
        }
        ...
        if (reconnecting) {
            ...
            if (info != null && info.isConnected()) {
                mHandler.postDelayed(new Runnable() {
                    @Override public void run() { if (mReconnecting) connect(); }
                }, mAutoReconnectDelay);
            } else {
                try {
                    ...
                    Log.e(TAG, "Register receiver fail", e);
                }
            }
        } else {
    ...
    public boolean configureExtras(Bundle extras) throws AudioException {
        ...
        if (mAudioHandler != null && mAudioHandler.isInitialized()) {
            createAudioHandler();
            Log.i(TAG, "Audio reloaded after settings change");
        }
        return needReconnect;
    }

    @Override public void onBluetoothScoConnected() {
    ...
    @Override public void onBluetoothScoDisconnected() {
    ...
    private AudioHandler getAudioHandler() throws NotSynchronizedException {
        ...
        if (mAudioHandler == null && mConnectionState == ConnectionState.CONNECTED)
            throw new RuntimeException("Audio missing while connected");
    ...
    private ModelHandler getModelHandler() throws NotSynchronizedException {
        ...
        if (mModelHandler == null && mConnectionState == ConnectionState.CONNECTED)
            throw new RuntimeException("Model missing while connected");
